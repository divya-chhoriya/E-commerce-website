...
var carts = document.querySelectorAll('.add-latest-cart');  /*returns all elements in the document that matches a specified CSS selector*/
var carts2 = document.querySelectorAll('.add-women-cart');
var carts3 = document.querySelectorAll('.add-men-cart');
var carts4 = document.querySelectorAll('.add-decor-cart');
...
//Details of products displayed on home decor's page
let home_decor_products = [
	{
		name: 'White Dial Wooden 35.56 cm Handcrafted Analogue Wall Clock',
		tag: 'clock1',
		price: 664.00,
		inCart: 0,
		desc: "Dial colour: White, Clock colour: Green and brown, Type: Wall clock, Display: Analogue, Features: Wooden, handcrafted, flower-shaped"
	},
	{
		name: 'Grey & Green Round Colourblocked 30.4 cm Analogue Wall Clock',
		tag: 'clock2',
		price: 614.00,
		inCart: 0,
		desc: "Colour: Grey, Type: Analogue Wall Clock, Design: Regular, Pattern: Colourblocked, Shape: Round, Theme: Contemporary, Material: Plastic"
	},
	{
		name: 'Hosley Decorative Metal Studded Round Wall Mirror',
		tag: 'mirror1',
		price: 3195.00,
		inCart: 0,
		desc: "Colour: Silver, Shape: Round, Type: Wall hanging, Frame material: Wooden, Features: Hand-painted"
	},
	{
		name: 'Set of 3 Red & Blue Buddha Monk Statue Figurines Showpiece',
		tag: 's1',
		price: 959.00,
		inCart: 0,
		desc: "Set content: Three showpieces, Colour: Red, blue and purple, Pattern: Buddha Monk Statue Figurines Showpiece"
	},
	{
		name: 'Set of 3 White Warli Hand-Painted Terracotta Vases',
		tag: 's2',
		price: 1401.00,
		inCart: 0,
		desc: "Set Content: 3 flower vases, Colour: White, Pattern: Warli hand-painting, Shape: Mini triplets pots, Material: Teracotta"
	},
	{
		name: 'Yellow Scented Candle',
		tag: 'a1',
		price: 611.00,
		inCart: 0,
		desc: "Colour: Yellow, Set content: 1 scented candle with jar, Pattern: Solid, Size: Regular, Shape: Spherical"
	},
	{
		name: 'Purple & White Floral Bouquet Wallflowers Fragrance Plug',
		tag: 'a2',
		price: 1499.00,
		inCart: 0,
		desc: "Purple & White Floral Bouquet Wallflowers Fragrance Plug, Disclaimer: The fragrance plug does not come with the refill. Use with Bath & Body Works fragrance refills only."
	},
	{
		name: 'Set of 3 Purple Lavender Fields Fragrance Jar Candles',
		tag: 'a3',
		price: 547.00,
		inCart: 0,
		desc: "Set Content: 3 jar candles, Colour: Purple, Fragrance: Lavender Fields, Pattern: Solid, Shape: Jar"
	}
];
...
for(let i=0; i<carts4.length; i++) {
	carts4[i].addEventListener('click', () => {
		cart_number(home_decor_products[i]);
		TotalCost(home_decor_products[i]);
	})
}
...
function setItems(product) {
	...
	cartItems = JSON.parse(cartItems);   /*JSON is used to exchange data to/from a web server*/  /*JavaScript object*/
...
function TotalBill()
{
    let productContainer = document.querySelector(".totBill");
	let cartCost = localStorage.getItem('TotalCost');
    productContainer.innerHTML += `
			<div class="FinalTotalContainer">
				<h4 class="grandTotal"><span>&#8377</span>${cartCost}</h4>
...
function clear_cart() {
	...
	window.open('e-commerce (home).html');
}
...
//Display details of products (through modal box) on home decor's page by clicking on that image
function show_details_decor(num) {
	let s = document.getElementById("show");
	let name = document.createTextNode("Product Name: ");
	s.appendChild(name);
	let h = document.createTextNode(home_decor_products[num].name);
	s.appendChild(h);
	b = document.createElement("br");
	s.appendChild(b);
	b = document.createElement("br");
	...
	let price = document.createTextNode("Price: ");
	s.appendChild(price);
	let p = document.createTextNode("Rs. " + home_decor_products[num].price + ".00");
	s.appendChild(p);
	b = document.createElement("br");
	s.appendChild(b);
	b = document.createElement("br");
	...
	let desc = document.createTextNode("Description: ");
	s.appendChild(desc);
	let d = document.createTextNode(home_decor_products[num].desc);
	s.appendChild(d);
}
...
var wish2 = document.querySelectorAll('.add-wishlist2');
var wish3 = document.querySelectorAll('.add-wishlist3');
var wish4 = document.querySelectorAll('.add-wishlist4');
...
for(let i=0; i<wish2.length; i++) {
	wish2[i].addEventListener('click', () => {
		Wishh(women_section_products[i]);
		pdt_wishh(women_section_products[i])
	})
}

for(let i=0; i<wish3.length; i++) {
	wish3[i].addEventListener('click', () => {
		Wishh(men_section_products[i]);
		pdt_wishh(men_section_products[i])
	})
}

for(let i=0; i<wish4.length; i++) {
	wish4[i].addEventListener('click', () => {
		Wishh(home_decor_products[i]);
		pdt_wishh(home_decor_products[i])
	})
}
